...
class RoadDataset(Dataset):
    def __init__(self, file_path, aug_cfg_filepath=None, use_aug=True, mode='train', max_iter=None):
        self.data_fns = pd.read_csv(file_path, header=0)
        if max_iter is not None:
            self.max_iter = int(max_iter)
            idx = random.choices(range(0, self.data_fns.shape[0]), k=self.max_iter)
            self.data_list = self.data_fns.loc[idx].values.tolist()
        ...
        if aug_cfg_filepath is not None:
            ...
            self.chip_size = [128, 128] #[256,256]  
        else:
            self.use_aug = False
    ...
    def open_image_and_mask(self, msi_path, sar_path, hsi_path, mask_path):
        ...
        image_list = []
        image_list.append(msi_fp)
        image_list.append(sar_fp)
        image_list.append(hsi_fp)
