...
class HighResolutionNet(nn.Module):
    ...
    def __init__(self, input_channels, backbone='hr-w18'):
        ...
        if backbone == 'hr-w18':
            self.filters = [18, 36, 72, 144]
        elif backbone == 'hr-w32':
            self.filters = [32, 64, 128, 256]
            # print("############")
        elif backbone == 'hr-w48':
            # print("!!!!!!!!!")
            self.filters = [48, 96, 192, 384]
        else:
            raise ValueError("Invalid indicator for backbone, which should be selected from {'hr-w18', 'hr-w32', 'hr-w48'}")
        ...
        self.stage4, pre_stage_channels = self._make_stage(
            3, num_channels, multi_scale_output=True)
        # print(np.sum(pre_stage_channels),"pre_stage_channels")
    ...
    def _make_stage(self, layer_no, num_inchannels,
                    multi_scale_output=True):

        num_modules = [1, 1, 4, 3]
        num_branches = [1, 2, 3, 4]
        num_blocks = [[4], [4, 4], [4, 4, 4], [4, 4, 4, 4]]
        num_channels = [self.filters[0], self.filters[0:2], self.filters[0:3], self.filters[0:4]]
        blocks = ['BOTTLENECK', 'BASIC', 'BASIC', 'BASIC']
        fusemethods = ['SUM', 'SUM', 'SUM', 'SUM']

        num_modules = num_modules[layer_no]
        num_branches = num_branches[layer_no]
        num_blocks = num_blocks[layer_no]
        num_channels = num_channels[layer_no]
        block = blocks_dict[blocks[layer_no]]
        fuse_method = fusemethods[layer_no]
...
if __name__ == '__main__':
    ...
    net.init_weights('/Top2-Seg-HRN/models/backbone/pretrained/hrnetv2_w18_imagenet_pretrained.pth')
