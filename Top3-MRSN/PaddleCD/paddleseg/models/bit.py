...
from paddleseg.models.layers.blocks import Conv3x3, Conv1x1, get_norm_layer, Identity
from paddleseg.models.backbones import resnet, hrnet

@manager.MODELS.add_component
class BIT2_UP(nn.Layer):
    """
    ...
    def __init__(self,
        ...
        # self.backbone_upsample = nn.Upsample(scale_factor=2)
        # self.backbone_conv_out = Conv3x3(self.backbone.num_channels[-1], EBD_DIM)
        ...
        self.conv_aug = nn.Conv2DTranspose(EBD_DIM, num_classes, kernel_size=2, stride=2)
    ...
    def forward(self, t1, t2):
        ...
        pred = F.interpolate(
                ...
                mode='bilinear',
                align_corners=False)
        pred_aug = self.conv_aug(y_)
        return [pred, pred_aug]
...
class BIT2(nn.Layer):
    ...
    def __init__(self,
        ...
        self.conv_out = nn.Sequential(
            Conv3x3(
                EBD_DIM, EBD_DIM, norm=True, act=True),
            Conv3x3(EBD_DIM, num_classes))
        self.conv_aug = nn.Conv2DTranspose(EBD_DIM, num_classes, kernel_size=4, stride=4)
    ...
    def forward(self, t1, t2):
        ...
        pred = F.interpolate(
                ...
                mode='bilinear',
                align_corners=False)
        aug_pred = self.conv_aug(y_)
        return [pred, aug_pred]
...
class BIT_act(nn.Layer):
    ...
    def forward(self, t1, t2):
        ...
        # x1 = self.backbone(t1)[-1]
        # x1 = self.backbone_upsample(x1)
        # x1 = self.backbone_conv_out(x1)
        # x2 = self.backbone(t2)[-1]
        # x2 = self.backbone_upsample(x2)
        # x2 = self.backbone_conv_out(x2)
        x1 = self.backbone(t1)
        x2 = self.backbone(t2)
...
class Backbone(nn.Layer, KaimingInitMixin):
    def __init__(self,
        ...
        if arch == 'resnet18':
            ...
            self.hrnet = hrnet.hrnet18(pretrained=True)
        else:
            raise ValueError
        ...
        if arch == 'resnet50' or arch == 'resnet101':
            ...
            itm_ch = self.hrnet.feat_channels[0]
...
class Backbone_UP(nn.Layer, KaimingInitMixin):
    def __init__(self,
                 in_ch,
                 out_ch=32,
                 arch='resnet18',
                 pretrained=True,
                 n_stages=5):
        super(Backbone_UP, self).__init__()
